...
class SessionManager:
    ...
    def __init__(self):
        ...
        # 原版 max_retries=10 只覆盖连接类错误，读超时(ReadTimeout)不会重试，
        # 而超星的视频进度上报接口恰恰经常读超时。这里显式配置重试策略：
        # 连接错误重试 3 次，读超时重试 2 次，指数退避，避免瞬间打爆服务端。
        _retry = Retry(
            total=3,
            connect=3,
            read=2,
        ...
        # 超时拆成 (连接, 读取)。原来统一 5 秒，超星进度上报偶尔要 8~15 秒才回，
        # 于是被判定 ReadTimeout 抛异常。放宽到 5s 连接 / 20s 读取。
        self._session.request = functools.partial(self._session.request, timeout=(5, 20))
        # For debug purposes
