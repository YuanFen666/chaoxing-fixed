...
def resource_path(relative_path: str) -> str:
    ...
    查找顺序（第一个存在的胜出）：
      1. PyInstaller 解包目录 (_MEIPASS) —— 正常打包进 exe 的资源在这里
      2. exe / 脚本所在目录          —— 方便把 resource/ 放在 exe 旁边做“外挂”修复
      3. 当前工作目录                —— 源码直接运行时的场景

    只按原来的 _MEIPASS 找，一旦打包漏了资源就彻底没救
    （官方 v3.1.4 的 exe 就是漏了 resource/font_map_table.json，
     导致加密字体的题干显示成乱码）。加上后两级回退以后，
     即使 exe 里没带，只要把 resource/font_map_table.json 放到 exe 同目录也能生效。

    Args:
        relative_path: 相对路径

    Returns:
        资源文件的绝对路径（不保证存在，由调用方处理 FileNotFoundError）
    """
    candidates = []
    ...
    # 都不存在时，返回第一个候选路径，让调用方抛出明确的 FileNotFoundError
    if candidates:
